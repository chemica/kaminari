...
The chemica/kaminari fork addresses an apparent problem with ActiveRecord, where queries with group_by clauses relying on column aliases
fail on [relation].count. The standard count query replaces the select clause with columns pulled fromm the group, meaning the aliases are
never defined and the query fails.

This fork will use a sub query for count in this situation. 
